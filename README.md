...
| Binance 워커 | `run_binance.py` | 20초 폴링 → 5분봉 ETC 급락 매수 후보 · 4시간봉/일봉 BTC 급등 추종 · 일봉 ETC 급락 추종 숏 · **거래대금 상위 30 코인 급변 감시** → 텔레그램 (토스 엔진과 독립, 3.5절 · 자동매매 dry/live 는 3.6절) |
...
| review | 🔵 매수 대기(확인 항목이 모자란 신호와 그 반복) · 🔴 매도 대기(얕은 이탈 신호와 미청산 반복) · 🟡 절반/1/3 익절 검토 · ⚪ 매수 대기 취소/만료 · ⚪ 청산 신호 해제 · 🎉✅ 청산 완료 · 📈 급등 확인 관찰 · 📉 급락 확인 관찰(Binance) · 🚀 급등 감지 · 💥 급락 감지(급변 감시, 관찰) | 45분 / 15분 (Binance 추종은 7일/20일, 급변 감시는 코인당 4시간) |
...
alertbot/
  ...
  binance_scan.py    Binance 선물 급변 감시 — 거래대금 상위 30 코인(백오피스에서 추가·제외) 1시간봉 급등·급락 감지 알림, 관찰 전용
...
**급변 감시 (거래대금 상위 30 코인, 1시간봉, 종류 `SCAN_SURGE` 🚀 / `SCAN_CRASH` 💥, 관찰 알림 — `binance_scan.py`)**
...
| 쿨다운·묶음 | 알린 코인은 방향과 무관하게 4시간 쉰다. 한 사이클에 같은 방향으로 여러 코인이 뜨면 알림 하나(배수 순 10줄 + '외 N종목') | `SCAN_MAX_LINES` |

알림 줄은 `코인 가격 (마감 시각) · 4시간 저점 대비 +x% (기준ATR y배) · RVOL · 거래대금 순위` 이고, **매매·가상 장부와 연결하지 않는다**(본문에 "관찰 알림, 매매 신호 아님").
코인 시황에는 한 줄만 싣는다: `급변 감시 1시간봉 상위30  감시 수 · 최대 상승 · 최대 하락 | 24시간 감지 N종목`. 조회는 코인마다 새 1시간봉이 끝났을 때만 한다(시간당 약 160 weight).
...
- **매매에 연결하지 않는 이유**: 알림 뒤 급등 추종 롱(4h·24h)·눌림 재돌파 롱(12h·24h)·급락 반전 롱(8h, 손절 −3%·없음)·급등 페이드 숏(8h 손절 +5%·24h 손절 없음) 8개 조합을
  수수료·거래대금 순위별 슬리피지·실제 펀딩·마크 가격 손절까지 넣어 검증했지만, 채택 조건(30건·20일 이상, 하루 단위 t ≥ 2.5, 두 절반 양수, 최대 기여 코인·날을 빼도 양수)을 넘은 조합이 없었다
  (t −1.2~1.1, 보유 중 최대 역행 p90 13~54%).
...
| 항목 | 규칙 | 상수 |
|---|---|---|
| 크기 | 명목가 = 전략별 배분 자본 × 유효 배율. 급락 매수 2 · 4시간봉 급등 추종 1 · 일봉 급등 추종 1.5 · 일봉 급락 숏 0.5 (분석의 시작값, 최대 3 / 1.5 / 2 / 1). 자본은 가상 장부 `ALERT_BINANCE_TRADE_CAPITAL`, live 는 계정별 | `BINANCE_TRADE_LEVERAGE` |
...
| 한도 | 장부마다: 전략당 열린 포지션 하나 · 합산 명목 ≤ 자본 합 × 3 · 오늘(KST) 실현손실이 자본 합의 6% 를 넘으면 신규 진입 중단 | `BINANCE_TRADE_MAX_TOTAL_LEV`, `BINANCE_TRADE_DAILY_LOSS_PCT` |
...
급변 감시(유니버스 필터·추가/제외·갱신 주기·실패 시 유지, 급등·급락·묶음·코인 쿨다운·시간 게이트·시황 줄), 관리 프로세스(재시작 대기·요청·감시견·절전 복귀·lease·종료,
