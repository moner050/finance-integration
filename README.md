...
손절 참고선(신호봉 종가 −3%, `CRASH_STOP_PCT`)과 보유 한도(5시간, `CRASH_HOLD_HOURS`), 낙폭의 50% 되돌림선을 함께 싣는다. 목표 지정가는 두지 않는다.
...
2. 진입·종료 경로 확인. 과거에 실제로 신호가 났던 봉을 워커에 먹여 알림 → 📥 진입 → 감시 → 📤 종료까지 돌린다. 봉은 Binance 에서 받아 오고,
   ...
   확인할 것: 알림 본문의 손절 참고선과 📥 메시지의 손절이 같은지, 명목 = 자본 × 배율인지(펀딩 게이트에 걸리면 절반), 보유 한도 시각이 사양(5시간·7일·20일)과 맞는지.
