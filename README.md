...
- **신호 포지션**: '매수하세요'(확인 항목 3개 이상)가 나가면 내 계좌에 보유가 없어도 그 종목은 곧바로 **보유**로 관리한다 — 다음 알림은
  매수 대기·취소·만료가 아니라 추가매수·익절·손절이다. 손익과 손절 한도(-5%)의 기준은 **신호가**이고(본문 첫 줄 `신호가 X 대비 ±y%`), 내 평단
  손익은 계좌 줄에만 붙는다(미보유면 `내 계좌 미보유 — 신호가 기준`). 확정 청산 신호(손절·매도·전량 익절·마감 정리)가 나가면 신호 포지션은 끝난다
  — 내 보유가 있으면 팔 때까지 청산대기(반복), 없으면 곧바로 관망. '매수 대기하세요' 는 확인 항목이 모자란 신호에만 쓰고, 채워지면 '매수하세요' 로
...
- **신호 모의 성적**: 신호 포지션이 닫힐 때마다 신호가 → 청산 신호 시점 가격의 손익이 `signal_trades.csv` 에 남고, 장 마감 때
  ...
  내 계좌의 실제 청산은 따로 `📈 오늘 성적`(trade_log.csv)이다. 기본은 내 채널에만 가고, 공개 채널로 보내려면 `models.PUBLIC_KINDS` 에 `SIGNAL_REPORT` 를 넣는다.

### 1.2 종목별 상태기계

`관망 → 진입대기(매수 대기 알림) → 보유(확정 매수 신호 = 신호 포지션, 또는 실제 보유 확인) → 청산대기(매도 알림) → 관망(확정 청산 신호 · 보유 소멸)`.
확정 매수 신호는 곧바로 보유(신호 포지션)다. 그 밖의 전이는 시간이 아니라 **실제 보유 변화**로 일어난다 — 내 보유가 있는데 알림대로 안 움직이면
같은 청산 알림이 쿨다운 간격으로 반복되고(이 반복과 '청산 신호 해제'는 내 계좌 일이라 내 채널에만 간다), 내 보유 없이 신호 포지션만 열려 있으면
확정 청산 신호 한 번으로 끝난다(반복할 대상이 없다).
내 보유가 사라져도 신호 포지션이 열려 있으면 관망으로 가지 않는다(내 계좌만 정리된 것). 상태·손절선·타이머·신호 포지션은 MySQL 에 저장되어 재시작해도 이어진다.
...
| review | 🔵 매수 대기(확인 항목이 모자란 신호와 그 반복) · 🔴 매도 대기(얕은 이탈 신호와 미청산 반복) · 🟡 절반/1/3 익절 검토 · ⚪ 매수 대기 취소/만료 · ⚪ 청산 신호 해제 · 🎉✅ 청산 완료 · 📈 급등 확인 관찰 · 📉 급락 확인 관찰(Binance) | 45분 / 15분 (Binance 추종은 7일/20일) |
| info | 📊 시황(30분) · 🔔🔕 장 시작/마감 · 📈 오늘 성적(내 계좌) · 📈 오늘 신호 성적(모의, 코인은 자정 KST) · ⚪ 시스템 | 없음 |
...
| `TELEGRAM_PUBLIC_BOT_TOKEN` / `TELEGRAM_PUBLIC_CHAT_ID` | | 공개 채널. 시장 신호(매수·취소·매도선 이탈·익절·불타기·마감 정리·코인 신호)와 장 시작·마감·시황을 보내되 손익·평단·보유 수량·내 포지션·손절 한도·청산 완료·주문은 보내지 않는다 (`models.PUBLIC_KINDS`, `Signal.account`). 확정 청산 뒤 내 보유가 남아 반복되는 '매도 대기'와 '청산 신호 해제'도 내 채널만(`Signal.private`) | 없음 |
...
새 급락 봉은 `🔵 급락 추가매수 후보` 로 부르고 손절선·보유 한도를 그 봉 기준으로 갱신한다. 장부는 자동매매(3.6, 내 계좌)와 독립이며
...
| `dry` | 만듦 | `DryRunBroker` (API 호출 없음, 지정가/참조가로 즉시 가상 체결). 가상 체결은 **엔진 보유에 합쳐진다** (모의매매) | 실제와 동일 | 의도·거절 사유·가상 체결 | `[DRY]` 접두어로 주문 접수/체결 알림 + 청산 완료·오늘 성적 |
...
- **가상 체결은 보유가 된다**: 체결된 dry 의도를 누적한 모의 보유(`db.dry_positions`, 매수는 가중 평단·매도는 수량 차감)를 엔진이 매 사이클
  실제 보유에 합친다(같은 종목을 실제로도 들고 있으면 모의 보유가 우선). 그래서 산 종목은 '보유' 로 관리되어 손절·매도·익절 신호에 전량 시장가 모의 매도가 나가고,
  보유가 사라지면 청산 완료(`※ 자동매매 체결가 기준`)와 `trade_log.csv`, 장 마감 `📈 오늘 성적` 까지 이어진다. 재시작해도 DB 의 체결 기록에서 다시 계산된다.
- 매도 신호 → 보유 수량 전량 시장가 의도, 참조가(신호 시점 현재가)로 가상 체결, 평단 대비 실현손익 계산.
- 잔고는 무한대로 본다. 수수료·슬리피지는 없다 — 규칙대로 사고팔았을 때의 결과를 보는 모의매매다. 백오피스 '자동매매' 화면의 '모의 보유' 가 현재 모의 포지션이다.
- 킬 스위치와 종목별 '자동매매' 체크 + 1회 매수 금액은 dry 에도 필요하다 (4.1). 꺼져 있으면 의도가 `disabled`/`symbol-not-auto` 로 거절되고 알림만 나간다.
...
| 화면 | 경로 | 기능 |
|---|---|---|
| 상태 | `/` | 마지막 사이클 시각(90초 넘으면 "멈췄을 수 있다" 경고), 감시 중·프리마켓 종목, 종목별 상태·현재가·종가·VWAP(밴드)·위치(현재가/종가)·RVOL(직전→현재, 방식)·정점·선행·손절선. 30초 자동 갱신. 아래에 **시황 시계열**(30분 시황을 종목 × 시각으로, 주식·코인) |
| 종목 | `/watchlist` | 추가/편집/중지/재개/삭제. **심볼 검증** 버튼은 토스 현재가 API 로 심볼·선행 종목을 실제 조회한다. 페어 정합성 `!` 경고 |
| 알림 | `/signals` | `alert_signal_log` 최근 200건. 종목·등급 필터, 채널별 전송 결과(ok / error / skip). 아래에 텔레그램 채널 설정 상태와 **테스트 발송**(쿨다운·등급 무시, 이력에 남음) |
| 자동매매 | `/trading` | 모드 표시, 킬 스위치, 한도 편집, 모의 보유(dry), 주문 의도 200건(상태·사유·주문번호·체결·손익), 미결 수동 취소, **주문 권한 확인** |
| 매매 결과 | `/results` | 위부터 **실전(live) 체결 결과**, **dry 모의매매 결과**, **신호 모의 성적**. 각각 청산 건수·승률·누적 손익과 일별 시계열(통화별 손익·누적), 건별 목록(평단·체결가·수익률) |
