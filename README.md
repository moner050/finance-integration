...
| 등급 | 알림 | 쿨다운 |
|---|---|---|
| action | 🔵 매수 · 🔴 손절/매도 · 🟢 전량 익절 · 🔵 추가매수 · 🟠 마감 정리 · 📤✅🚫⛔ 주문 관련 · 🔵 급락 매수/추가매수 후보 · 🔵 눌림 재돌파 진입 후보 · 🔴 반등 실패 숏 후보 · 🔴 손절/🟢 전량 익절·정리(코인 신호 포지션 종료) · 📥📤 가상 포지션 진입/종료(Binance) | 15분 (주문 관련은 없음, Binance 급락 매수 60분 · 추종은 보유 한도 7일/20일) |
| review | 🔵 매수 대기(확인 항목이 모자란 신호와 그 반복) · 🔴 매도 대기(얕은 이탈 신호와 미청산 반복) · 🟡 절반/1/3 익절 검토 · ⚪ 매수 대기 취소/만료 · ⚪ 청산 신호 해제 · 🎉✅ 청산 완료 · 📈 급등 확인 관찰 · 📉 급락 확인 관찰(Binance) · 🚀 급등 감지(가상 장부 소진 숏 대기) · 💥 급락 감지(관찰) | 45분 / 15분 (Binance 추종은 7일/20일, 급변 감시는 코인당 4시간) |
...
알림 줄은 `코인 가격 (마감 시각) · 4시간 저점 대비 +x% (기준ATR y배) · RVOL · 거래대금 순위` 다. 급등 알림은 아래 **급등 소진 숏**(가상 장부 전용)으로 이어지고 본문에 그 규칙이 적힌다. 급락 알림은 관찰 알림이다(본문에 "관찰 알림, 매매 신호 아님").
코인 시황에는 한 줄만 싣는다: `급변 감시 1시간봉 상위30  감시 수 · 최대 상승 · 최대 하락 | 24시간 감지 N종목 · 숏 대기 N종목`. 조회는 코인마다 새 1시간봉이 끝났을 때만 한다(시간당 약 160 weight).
...
**신호 포지션 장부 (`binance_book.py`)** — 진입 후보(🔵 급락 매수 · 🔵 눌림 재돌파 · 🔴 반등 실패 숏)가 나가면 신호가·손절선·보유 한도를 장부에
적고, 이후 그 심볼의 알림은 진입이 아니라 관리다: 20초마다 마크 가격을 보고 손절선에 닿으면 🔴 손절하세요(`SELL`), 보유 한도가 되면
🟢 전량 익절/정리하세요(`EXIT_FULL`)를 보내고 장부에서 지운다(숏은 '숏 손절'/'숏 전량 익절'). 코인 시황의 해당 줄은 조건 충족 여부 대신
`🔵 매수 신호 진행 중 — 신호가 대비 ±% (마크) · 손절 · 한도` 로 바뀐다. 5분봉 급락 매수는 60분 쿨다운이 8시간 보유보다 짧아, 신호가 진행 중일 때의
...
`run_binance.py` 가 진입 후보 알림(🔵 급락 매수 후보 · 🔵 눌림 재돌파 진입 후보 · 🔴 반등 실패 숏 후보)과 급변 감시의 급등 소진 숏(3.5절, 가상 장부 전용)을 체결하고 열린 포지션을 20초마다 감시한다.
...
코인 시황(공용 채널)에는 가상 장부의 포지션만 '가상 포지션 (dry)' 로 실린다. 계정 live 포지션은 그 계정의 📥/📤 와 자정(KST) `📈 오늘 코인 성적` 으로만 본다.
