...
def test_summary_page_builds_time_series(client):
    ...
    # 신호 포지션 진행 줄 (주식·코인) 은 '진행 중' 으로 요약된다
    live = A.parse_summary("🔵 삼성전기  매수 신호 진행 중 — 신호가 1344000.0 대비 +0.22%, 기준선 위\n"
                           "급락 매수 5분봉 ETCUSDT  🔵 매수 신호 진행 중 — 신호가 7.100 대비 +0.70% (마크 7.150) · 손절 6.887 · 한도 09-16 20:00 KST 까지")
    assert live["items"]["삼성전기"]["short"] == "매수 신호 진행 중" and live["items"]["삼성전기"]["mark"] == "🔵"
    assert live["items"]["급락 매수 5분봉 ETCUSDT"]["short"] == "🔵 매수 신호 진행 중" and "손절 6.887" in live["items"]["급락 매수 5분봉 ETCUSDT"]["detail"]
    r = c.get("/summary")
